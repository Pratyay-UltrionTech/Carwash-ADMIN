...
/** Escapes a value for an Excel-compatible CSV and prevents formula execution. */
...
export default function CustomersPage() {
  ...
  const exportCustomers = async () => {
    ...
    try {
      const perPage = 200;
      const first = await adminListCustomers({ search, typeFilter, sortBy, page: 1, perPage });
      const rows = [...first.customers];
      const exportPages = Math.ceil(first.total / perPage);

      for (let exportPage = 2; exportPage <= exportPages; exportPage += 1) {
        const result = await adminListCustomers({ search, typeFilter, sortBy, page: exportPage, perPage });
      ...
      const csvRows = rows.map((customer) => [
        customer.type === 'account' ? 'Account' : 'Guest',
        customer.customer_id ?? (customer.guest_key ? `GUST_${guestCode(customer.guest_key)}` : ''),
        customer.name,
        customer.email,
        customer.phone,
        customer.vehicles.join('; '),
        customer.branch_booking_count,
        customer.mobile_booking_count,
        customer.total_booking_count,
        customer.last_booking_date,
        customer.free_wash_eligible ? 'Yes' : 'No',
      ]);
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          <button
            ...
            disabled={exporting}
            className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-white px-3 py-1.5 text-sm font-medium text-emerald-700 shadow-sm transition-colors hover:bg-emerald-50 disabled:opacity-50"
          >
              ...
              <tr key={c.customer_id ?? c.guest_key} className="group transition-colors hover:bg-slate-50/60">
                ...
                <td className="px-3 py-2.5 align-middle min-w-0">
                  <div className="flex min-w-0 items-center gap-1.5">
                    <p className="min-w-0 truncate font-medium text-slate-900" title={c.name || undefined}>{c.name || '—'}</p>
                    {c.free_wash_eligible && (
                      <span
                        className="inline-flex size-5 shrink-0 items-center justify-center rounded-full bg-amber-50 text-amber-600"
                        title="Eligible for a free wash reward"
                        aria-label="Eligible for a free wash reward"
                      >
                        <svg className="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 21h8m-4-4v4M7 4h10v4a5 5 0 01-10 0V4zm0 2H4v1a4 4 0 004 4m9-5h3v1a4 4 0 01-4 4" />
                        </svg>
